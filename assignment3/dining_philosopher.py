...
# All Necessary imports from the client and server and with respect to the fork addresses
                ...
                if data:
                    ...
                    if request[1] == 1:
                        if self.being_used:  # If fork is dirty/being used
                            ...
                            # For acquiring fork, action is 1 and in case of failure success is 0
                            connection.send(pickle.dumps([self.id, request[0], 1, 0]))
                            break
                        # This situation is in case the fork is not cleaned or being used    
                        else: 
                            self.being_used = True
                            self.being_used_by = request[0]  # Store the id of the philosopher who has the fork now
                            # Send [fork_id, philosopher_id, action, success]
                            # For acquiring fork, action is 1 and in case of success is 1
    ...
    def __init__(self, number_of_philosophers):
        ...
        for i in range(number_of_philosophers):
            ...
            if i != (number_of_philosophers - 1):
                ...
                self.display_format += "{: >20}"
            self.divider_line += "----------------------"
        print("\n\nDining Philosophers Problem (Socket Edition):\n\n")
